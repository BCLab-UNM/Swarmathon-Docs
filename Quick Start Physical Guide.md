...
Connect the Intel NUC onboard your robot to a keyboard (USB), mouse (USB), and monitor (HDMI). Ensure that the motors are turned off (the red switch is down), then turn the NUC on.

If your robot does not yet have an operating system (OS) installed on it, begin by installing the 64-bit Desktop version of [Ubuntu 14.04](http://releases.ubuntu.com/14.04/). The simplest method for installing Ubuntu is to use a USB flash drive following the [appropriate instructions](https://help.ubuntu.com/community/Installation/FromUSBStick) for your particular PC OS.
