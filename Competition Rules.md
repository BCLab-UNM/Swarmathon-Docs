...
- Teams may modify the Arduino code. Loading the Arduino code must be fully automatic and triggered from the `/misc/rover_onboard_node_launch.sh` script. *Arduino code must be stored in a subdirectory of the teams repository called `arduino/Swarmathon_Arduino/`*, i.e. `Swarmathon-TeamAbbrev/arduino/Swarmathon_Arduino/` where Swarmathon-TeamAbbrev is your repository name. 
